(() => {
  ...
  function append(entry) {
    ...
    line.className = entry.level === "ERROR" ? "text-red-400" : entry.level === "WARN" ? "text-amber-300" : "text-emerald-200/80";
  ...
  source.onopen = () => { const t = status.querySelector("[data-log-status-text]") || status; t.textContent = "● LIVE // FOLLOWING"; };
  source.onerror = () => { const t = status.querySelector("[data-log-status-text]") || status; t.textContent = "○ LINK LOST // RETRY"; };
