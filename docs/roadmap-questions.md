...
| GOAL-104 | Should an authenticated SRT ingest automatically publish to configured destinations, or should it enter the same staged-then-dashboard-published lifecycle as RTMP? | SRT is authenticated and remuxed into the RTMP ingest listener; destination publication follows the existing staged control path pending this product decision. |
