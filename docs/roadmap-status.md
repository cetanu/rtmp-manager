...
| 501 | Persistent monthly quotas with Free/Pro/Enterprise hour and destination enforcement, Stripe customer portal sessions, tenant usage snapshots, Plan Usage dashboard card, and generic, Stripe, and LemonSqueezy webhooks | Production account/customer lifecycle integration and subscription checkout provisioning |
...
| 503 | Administrator-only global and tenant emergency-stop endpoints, cross-tenant active stream status API, bounded audit-log API, append-only administrator audit log, and 60-second HLS JPEG thumbnail capture endpoint | Production authorization review and object-storage retention hardening |
