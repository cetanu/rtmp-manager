...
| 503 | Administrator-only global and tenant emergency-stop endpoints, cross-tenant active stream status API, bounded audit-log API, append-only administrator audit log, and incident runbook | Production authorization review and thumbnail snapshot workers |
